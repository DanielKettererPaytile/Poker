...
def main():
    ...
    class Game(object):
        def __init__(self):
            ...
            p0history.append(self.list_of_players[0].chips)
            p1history.append(self.list_of_players[1].chips)   
        ...
        def establish_player_attributes(self):
            address_assignment = self.starting_index
        ...
        def end_round(self):
            ...
            if new_round == "yes":
                ...
                self.starting_index %= len(self.list_of_players)
                # plt.close()
                # plt.plot(p0history)
                # plt.plot(p1history)
                # plt.legend([self.list_of_players[0],self.list_of_players[1]])
    ...
    class StartPage(Frame):
        ...
        def button_click(self, entry0, entry1,# entry2, entry3, entry4, entry5, entry6, entry7, entry8, entry9, 
                         entrysc, entrysb, entrybb, controller):
            entry_list = [entry0, entry1,# entry2, entry3, entry4, entry5, entry6, entry7, entry8, entry9,
                          entrysc, entrysb, entrybb]
                ...
                if starting_index == 0:
                    ...
                    try:
                        card2 = ImageTk.PhotoImage(
                            Image.open("cards\\" + str(game.list_of_players[starting_index].cards[1]) + ".png").resize((55, 85), Image.ANTIALIAS))
